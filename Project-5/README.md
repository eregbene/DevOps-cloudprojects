# Project-5: Continuous Integration on AWS Cloud
...
* AWS Account
* Sonar Cloud Account
* AWS CLI, Git installed on Local machine
...
[Project code](https://github.com/eregbene/vprofile-project-all)
...
Then we run command get token as in the instructions. this code is very important we use to get the dependenciesfor maven.
```sh
...
```sh
Organization: eregbene
Project key: eregbene
Public
```
...
We need to update sonar_buildspec.yml file paramter store sections with the exact names we have given in SSM Parameter store. note!!!( change the version for maven as they no longer suupport use 3.9.3 or check for the latest support)
...
env:
  secrets-manager:
    ...
    CODEARTIFACT_AUTH_TOKEN: sonar:CODEARTIFACT_AUTH_TOKEN
phases:
  install:
    runtime-versions:
      java: corretto8
