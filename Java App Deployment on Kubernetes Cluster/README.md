# Project-12: Java App Deployment on Kubernetes Cluster
...
My registered domain name: `eregbeneprojects.com`. I will create a hostedzone with below name. 

```sh
Name: eregbeneprojects.com
```
...
```sh
kops update cluster --name kubevpro.eregbeneprojects.com --state=s3://kubeapp-kops-state --yes --admin
```
...
aws ec2 create-volume \
    ...
    --volume-type gp2 \
    --size 3 \
    --tag-specifications 'ResourceType=volume,Tags=[{Key=KubernetesCluster,Value=kubevpro.eregbeneprojects.com}]'
```
...
```sh
https://github.com/eregbene/kube-app
```

In Project-11, we have Containerized the vprofile application and push it to the DockerHub. We will use the images below with :V1 tag from DockerHub repository in our K8s manifests.
...
```sh
kops delete cluster --name=kubevpro.eregbeneprojects.com \
